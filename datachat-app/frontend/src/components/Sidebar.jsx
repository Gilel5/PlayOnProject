...
export default function Sidebar({
  ...
  return (
    <aside ref={sidebarRef} className={`w-72 flex-shrink-0 border-r flex flex-col h-full ${darkMode ? "bg-black border-gray-700" : "bg-white border-gray-100"}`}>
      {/* Top bar */}
      <div className={`flex items-center justify-between px-4 py-4 border-b ${darkMode ? "border-gray-700" : "border-gray-100"}`}>
        ...
        <button onClick={onCreateChat} className={`p-1.5 rounded-lg hover:bg-gray-100 transition-colors ${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"}`}>
          <Plus size={18} className={darkMode ? "text-gray-300" : "text-gray-600"} />
        </button>
      </div>

      {/* Search */}
      <div className="px-3 py-3">
        <div className={`flex items-center gap-2 border rounded-lg px-3 py-2 ${darkMode ? "bg-gray-800 border-gray-600" : "bg-gray-50 border-gray-200"}`}>
          <Search size={14} className={darkMode ? "text-gray-400" : "text-gray-400"} />
          <input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search"
            className={`bg-transparent text-sm placeholder-gray-400 outline-none flex-1 min-w-0 ${darkMode ? "text-gray-200" : "text-gray-700"}`}
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-4">
        {filteredPinned.length > 0 && (
          <div>
            <p className={`px-2 py-1 text-xs font-semibold uppercase tracking-wide ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
              Pinned Chats
            </p>
              ...
              <div key={chat.id} className="relative group">
                {renamingId === chat.id ? (
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2.5 rounded-xl text-sm border border-[#5BC5D0] outline-none ${darkMode ? "bg-gray-800 text-white" : "bg-white"}`}
                  />
                ) : (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 rounded-xl text-sm transition-colors pr-10 ${
                      activeChatId === chat.id 
                        ? (darkMode ? "bg-gray-700 text-white font-medium" : "bg-gray-100 text-gray-900 font-medium") 
                ...
                {renamingId !== chat.id && (
                  ...
                  >
                    <MoreHorizontal size={14} className={darkMode ? "text-gray-400" : "text-black"} />
                  </button>
                )}
              </div>
            ))}
      ...
      {openMenu && activeMenuSession && (
        <div
          ...
          className={`w-36 border rounded shadow-lg z-50 ${darkMode ? "bg-gray-800 border-gray-600" : "bg-white border-gray-200"}`}
        >
          <button
      ...
      {/* User + settings */}
      <div className={`border-t px-3 py-3 flex items-center justify-between ${darkMode ? "border-gray-700" : "border-gray-100"}`}>
        <div className="flex items-center gap-2.5">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${darkMode ? "bg-gray-700" : "bg-gray-200"}`}>
            <User size={15} className={darkMode ? "text-gray-300" : "text-gray-600"} />
          </div>
          <span className={`text-sm font-medium truncate max-w-[120px] ${darkMode ? "text-white" : "text-gray-800"}`}>
        ...
        <button
          ...
          className={`p-1.5 rounded-lg hover:bg-gray-100 transition-colors ${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"}`}
        >
          <Settings size={16} className={darkMode ? "text-gray-300" : "text-gray-500"} />
        </button>
      </div>
    </aside>
  );
